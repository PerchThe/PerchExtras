...
public class TeleportWithBack extends FrameworkCommand {

    private final AddonManager addonManager = AntiBackTeleport.getAddonManager();

    public TeleportWithBack() {
        super("teleportwithback");
        this.setPermissions("teleportwithback.command");
    }
    ...
    public void onExecute(CommandExecutor cmd) {
        ...
        try {
            ...
            Bukkit.getScheduler().runTask(AntiBackTeleport.getInstance().plugin, () -> player.teleportAsync(location).thenAccept(success -> {
